...
class DecoderNN(torch.nn.Module) :
    def __init__(self, n_unique_chars, embedding_size, context_length, n_heads) :
        ...
        self.linear_2 = torch.nn.Linear(size, 250, bias=False)
        self.normalizer_2 = torch.nn.LayerNorm(250)
        self.activation_2 = torch.nn.Tanh()

        self.output = torch.nn.Linear(250, n_unique_chars)
...
checkpoint = torch.load('ATTENTION-1.ckpt', map_location=torch.device('cpu'))
